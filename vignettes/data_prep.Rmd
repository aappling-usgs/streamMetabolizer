...
of Bob Hall. Explicitly attached units are optional but useful for confirming
that the data are in the expected format.
```{r data, message=FALSE}
library(streamMetabolizer)
dat <- data_metab(num_days='3', res='15', attach.units=TRUE)
```

Inspect the dimensions, column names, and units of the data. Note that `unitted`
data.frames, such as the one returned by `data_metab(..., attach.units=TRUE)`,
prints out with a second row called 'U' giving the units for each column. Again,
you can use a regular old data.frame for your data, but make sure the units of
your data are the same as in this example.
```{r data_check}
dim(dat)
...
Also read through the help page at `?mm_data` for more detailed variable definitions and requirements.
